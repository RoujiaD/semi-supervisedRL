...
def make_model():
    ...
    model.compile(loss='logcosh', optimizer=keras.optimizers.Adam(lr=0.0005))
...
def q_iteration(steps, env, model, target_model, iteration, current_state,
    ...
    train_input_class.append(next_state)
    steps += 1
    ...
    return steps, action, reward, is_terminal, epsilon, current_state, score, scores, train_input_class
...
def main():
    ...
    for i in range(number_training_steps):
        ...
        steps, action, reward, is_terminal, epsilon, current_state, score, scores,train_input_class = q_iteration(
        ...
        if np.mean(scores[::-1][0:100]) > 6000:
