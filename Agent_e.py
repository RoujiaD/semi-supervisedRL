...
def Agent_q_iteration(steps, env, model, target_model, iteration, current_state, mem_states, mem_actions, mem_rewards,
    ...
    # If DONE, reset model, record score
    if is_terminal:
        env.reset()
...
def Agent(t,ratio):
    """ Train the DQN to play Cartpole with RNN trained using data generated in the beginning
        """
    ...
    for i in range(number_training_steps):
        ...
        if (i + 1) % print_progress_after == 0:
            ...
            plt.clf()
            plt.plot(true_scores, label='train')
            plt.ylabel('scores')
    ...
    for j in range(number_of_RNNmodels_class):
        his = MODELS[0][j].fit(np.array(train_input_class), np.array(train_output_class), batch_size=100, epochs=20, verbose=0)
    ...
    for i in range(number_testing_steps):
        ...
        steps, action, reward_pred, is_terminal, epsilon, current_state, score, scores, true_score, true_scores = \
            Agent_q_iteration(steps, env, model, target_model, iteration, current_state, mem_states, mem_actions, mem_rewards,
                              mem_terminal, mem_size, score, scores, true_score, true_scores, number_of_RNNmodels_class, number_of_RNNmodels_reg, MODELS)
        ...
        if (i + 1) % print_progress_after == 0:
            ...
            plt.plot(true_scores[0:number_training_steps + number_random_actions + 1])
            plt.title('Agent_e')
            plt.ylabel('scores')
    ...
    plt.ioff()
#   Save Agent_e
    file_name = os.path.join('Agents_0.06', 'Agent_e_5')
